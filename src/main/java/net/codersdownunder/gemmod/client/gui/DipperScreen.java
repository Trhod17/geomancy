...
public class DipperScreen extends AbstractContainerScreen<DipperMenu>
{
    ...
    private ResourceLocation GUI = new ResourceLocation(Geomancy.MODID, "textures/gui/dipper.png");
    ...
    public void render(PoseStack matrixStack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (mouseX >= this.leftPos + 152 && mouseX <= this.leftPos + 167 && mouseY >= this.topPos + 81 && mouseY <= this.topPos + 124) {
            ...
            tooltip.add(ent.getFluid().isEmpty() ? TextUtils.Tooltip("screen." + Geomancy.MODID + ".tank_empty") : ent.getFluid().getDisplayName());
            DecimalFormat f = new DecimalFormat("#,##0");
            tooltip.add(Component.translatable("screen." + Geomancy.MODID + ".liquid_amount", f.format(ent.getFluid().getAmount()), f.format(DipperBlockEntity.capacity)));
            float percentage;
            ...
            tooltip.add(Component.translatable("screen." + Geomancy.MODID + ".liquid_percentage", String.format("%.2f", percentage)).append("%").withStyle(percentage < 60 ? ChatFormatting.GREEN : percentage < 90 ? ChatFormatting.YELLOW : ChatFormatting.RED));
